...
export const TourSlide: React.FC<TourSlideProps> = ({
    ...
    return (
        <div
            className={`
                w-screen h-full snap-start flex-shrink-0 relative overflow-hidden
                ${themeStyles[theme]}
                ${className}
            `}
        >
            ...
            {/* Content Layer - no bottom padding, buttons are fixed positioned */}
            <div className="relative z-10 h-full w-full flex flex-col justify-center items-center px-4 md:px-6">
                {children}
            </div>
        </div>
    );
